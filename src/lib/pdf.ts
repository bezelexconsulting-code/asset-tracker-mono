...
async function dataUrlToJPEGBytes(dataUrl: string): Promise<{ bytes: Uint8Array; width: number; height: number }> {
  // If already JPEG, decode directly; otherwise draw to canvas and convert to JPEG
  ...
  if (!match) {
    // Fallback: create tiny blank JPEG
  ...
  // If width/height not set, probe by loading into Image
  if (width === 0 || height === 0) {
    const img = new Image();
    img.src = dataUrl;
    await img.decode().catch(() => {});
...
export async function generatePDF(options: {
  ...
  const imgObjs: Array<{ objIndex: number; name: string; w: number; h: number }> = [];
  let imgCount = 0;
  for (const dataUrl of imgs) {
    const { bytes, width, height } = await dataUrlToJPEGBytes(dataUrl);
    const name = `Im${imgCount}`;
    const stream = `<< /Type /XObject /Subtype /Image /Width ${width} /Height ${height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${bytes.length} >>\nstream\n`;
    const footer = '\nendstream\n';
    const content = stream + Array.from(bytes).map((b) => String.fromCharCode(b)).join('') + footer;
    addObject(`4 ${imgCount} obj\n${content}endobj\n`); // place-holder numbering; we'll renumber later
    imgObjs.push({ objIndex: objects.length, name, w: width, h: height });
    imgCount++;
  }

  // Font object
  const fontObjNum = objects.length + 5; // approximate; we will fix references based on order
  addObject(`${fontObjNum} 0 obj\n<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>\nendobj\n`);

  // Build contents per page with pagination
  const margin = 40;
  const colW = (A4.w - margin * 2 - 20) / 3; const colH = 90; const gap = 10;
  const imgsPerRow = 3; const rowsAvailable = 4; // approx; header consumes space
  const imgsPerPage = imgsPerRow * rowsAvailable;
  const pages: string[] = [];
  const totalPages = Math.max(1, Math.ceil(imgObjs.length / imgsPerPage));
  for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
    ...
    const start = pageIndex * imgsPerPage;
    const slice = imgObjs.slice(start, start + imgsPerPage);
    let col = 0; let row = 0; let imIndex = start;
    for (const _ of slice) {
      const x = margin + col * (colW + gap);
      const imgW = colW; const imgH = colH;
      const yImgTop = A4.h - margin - 160 - row * (imgH + gap);
  ...
  // Resources dictionary referencing all images
  const xObjectEntries = imgObjs.map((_, i) => `/Im${i} ${4 + i} 0 R`).join(' ');
  const resObjNum = objects.length + 6;
  addObject(`${resObjNum} 0 obj\n<< /ProcSet [/PDF /Text /ImageC] /Font << /F1 ${fontObjNum} 0 R >> /XObject << ${xObjectEntries} >> >>\nendobj\n`);

  // Add content and page objects for each page
  ...
  pages.forEach((contents, idx) => {
    ...
    addObject(`${pageObjNum} 0 obj\n<< /Type /Page /Parent ${pageObjNum + (totalPages - idx) + 1} 0 R /MediaBox [0 0 ${A4.w} ${A4.h}] /Resources ${resObjNum} 0 R /Contents ${contentsObjNum} 0 R >>\nendobj\n`);
  ...
  addObject(`${catalogObjNum} 0 obj\n<< /Type /Catalog /Pages ${pagesObjNum} 0 R >>\nendobj\n`);

  // Rebuild xref with accurate offsets
  const body = objects.join('');
  const xrefPos = (header + body).length;
  const totalObjs = objects.length + 0; // already correct
  let xref = `xref\n0 ${totalObjs + 1}\n0000000000 65535 f \n`;
