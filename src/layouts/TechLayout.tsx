...
function Header() {
  ...
  const { brandingLogoUrl, brandingPrimaryColor } = useOrganization();
  return (
        ...
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 bg-gray-100 border border-gray-200 rounded overflow-hidden flex items-center justify-center">
            {(brandingLogoUrl || settings.branding.logo_url) ? (
              <img src={(brandingLogoUrl || settings.branding.logo_url)!} alt="Logo" className="w-full h-full object-cover" />
            ) : (
              <span className="text-[10px] text-gray-400">Logo</span>
            )}
          </div>
          <div className="font-semibold" style={{ color: (brandingPrimaryColor || settings.branding.primary_color || '#111827') }}>{settings.orgProfile.name || org}</div>
        </div>
