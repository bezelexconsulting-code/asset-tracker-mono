...
export function SuperAdminProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try { setState(JSON.parse(raw)); } catch { setState({ orgs: [], requests: [], invoices: [], tickets: [], flags: [], user_changes: [] }); }
    } else {
      const empty: SuperAdminState = { orgs: [], requests: [], invoices: [], tickets: [], flags: [], user_changes: [] };
      localStorage.setItem('bez-superadmin', JSON.stringify(empty));
      setState(empty);
    }
  }, []);
  useEffect(() => {
