  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {(() => { const assets = listAssets(undefined); const checked = assets.filter((a:any)=> a.status==='checked_out').length; return (
          <>
            ...
            <div className="bg-white border border-gray-200 rounded p-4">
              ...
              <div className="mt-2 text-2xl font-semibold">{checked}</div>
            </div>
          </>
        ); })()}
        <div className="bg-white border border-gray-200 rounded p-4">
          <div className="text-sm text-gray-500">Clients</div>
          <div className="mt-2 text-2xl font-semibold">{listClients().length}</div>
        </div>
        <div className="bg-white border border-gray-200 rounded p-4">
          <div className="text-sm text-gray-500">Technicians</div>
          <div className="mt-2 text-2xl font-semibold">{listTechnicians().length}</div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded p-6">
        ...
        <div className="mt-4 divide-y divide-gray-200">
          {(listActivities() as any[]).slice(0,10).map((a)=> {
            const asset = listAssets(undefined).find((x:any)=>x.id===a.asset_id);
            const client = listClients().find((c:any)=>c.id===a.client_id);
            return (
              <button key={a.id} className="w-full text-left py-3 flex items-center justify-between hover:bg-gray-50" onClick={()=> setSelectedAct(a)}>
                <span className="text-sm text-gray-700">{a.type} • {asset?.name || a.asset_id || ''} • {client?.name || ''}</span>
                <span className="text-xs text-gray-500">{new Date(a.created_at||'').toLocaleString()}</span>
              </button>
            );
          })}
          {(listActivities() as any[]).length===0 && <div className="text-sm text-gray-500">No activity</div>}
        </div>
      </div>
      {selectedAct && (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-full max-w-md px-4">
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="text-sm">Type: {selectedAct.type}</div>
