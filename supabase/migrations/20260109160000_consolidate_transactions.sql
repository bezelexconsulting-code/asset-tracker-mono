...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transactions_v2') THEN
    -- Insert without technician_id to avoid schema mismatches; it will default to NULL
    INSERT INTO transactions (id, org_id, asset_id, type, notes, created_at, from_location_id, to_location_id)
    SELECT id, org_id, asset_id, type, notes, created_at, from_location_id, to_location_id
